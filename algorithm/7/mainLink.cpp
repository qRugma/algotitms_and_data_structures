...
class VertexFinder{
    public:
        ...
        bool operator()(const Leader& leader)
        {
            return leader.VertexNumber == vertexNumber_;
        }
...
int main(){
    ...
    while (true){
        ...
        if (firstVertexIt == graph.end())
            graph.push_front(
                Leader{inVertex, 0, new LinkedList<Trailer>()}
            );

        Leader *pA = &(*graph.find_if(
            VertexFinder(inVertex)
        ));
